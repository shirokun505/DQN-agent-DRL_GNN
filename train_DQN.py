...
class DQNAgent:
    ...
    def _train_step(self, batch, fileLogs):
        ...
        with tf.GradientTape() as tape:
            ...
            for x in batch:
                ...
                # Calculate target value, ensuring it's a single tensor
                target_value = x[5] + self.gamma * tf.math.reduce_max(preds_next_target) * (1 - x[8])
                target.append(tf.stop_gradient(tf.convert_to_tensor(target_value)))  # Ensure target_value is a tensor

            # Calculate the loss using Mean Squared Error
            loss = tf.keras.losses.MSE(tf.stack(target, axis=0), tf.stack(preds_state, axis=0))

            # Apply L2 Regularization
            regularization_loss = sum(self.primary_network.losses)  # Ensure this sums the regularization losses from the model
            loss += regularization_loss

            # Convert loss to a scalar before logging
            loss_value = tf.reduce_mean(loss).numpy()  # Ensure it's a scalar
            fileLogs.write(".," + '%.9f' % loss_value + ",\n")  # Write loss to fileLogs

        # Computes the gradient using operations recorded in context of this tape
        grad = tape.gradient(loss, self.primary_network.trainable_variables)  # Use trainable_variables for clarity
        gradients = [tf.clip_by_value(gradient, -1., 1.) for gradient in grad if gradient is not None]  # Filter out None gradients
    ...
    def replay(self, episode):
        for i in range(MULTI_FACTOR_BATCH):
            ...
            grad, loss = self._train_step(batch, fileLogs)

            if i % store_loss == 0:
                fileLogs.write(".," + '%.9f' % loss + ",\n")

        # Hard weights update
        if episode % copy_weights_interval == 0:
            self.target_network.set_weights(self.primary_network.get_weights())

        # Optional: Uncomment if you want to log summaries at certain episodes
...
if __name__ == "__main__":
    ...
    # Fix: Create checkpoint with both model and optimizer
    checkpoint = tf.train.Checkpoint(
        model=agent.primary_network,
        target_model=agent.target_network,  # Added target network
